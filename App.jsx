  ...
  return (
    <div style={{ fontFamily: '-apple-system,BlinkMacSystemFont,"Segoe UI",system-ui,sans-serif', maxWidth: 480, margin: '0 auto', minHeight: '100vh', background: C.bg }}>
      ...
      <div style={{ padding: '16px 14px' }}>
        ...
        {tab === 'rewards' && <RewardsTab points={points} rewards={rewards} onClaim={cost => setPoints(p => p - cost)} />}
      </div>
    </div>
  )
}
...
function RewardsTab({ points, rewards, onClaim }) {
  const [flash, setFlash] = useState(null)
  function claim(reward) { if (points >= reward.cost) { onClaim(reward.cost); setFlash({ id: reward.id, type: 'claimed' }) } else { setFlash({ id: reward.id, type: 'short' }) } setTimeout(() => setFlash(null), 2500) }
  return <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
    <div style={{ background: `linear-gradient(135deg,#1a2030 0%,#0f1520 100%)`, border: `1px solid ${C.border}`, borderRadius: 20, padding: '20px 22px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><div><div style={{ color: C.textSec, fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1.2 }}>Available points</div><div style={{ color: C.textPri, fontSize: 44, fontWeight: 800, lineHeight: 1.1, marginTop: 2 }}>{points}</div></div><div style={{ fontSize: 44 }}>🏅</div></div>
    {rewards.map(reward => { const canAfford = points >= reward.cost; const isClaimed = flash?.id === reward.id && flash.type === 'claimed'; const isShort = flash?.id === reward.id && flash.type === 'short'; return <div key={reward.id} style={{ background: C.card, borderRadius: 20, overflow: 'hidden', border: isClaimed ? `1.5px solid rgba(34,197,94,0.4)` : isShort ? `1.5px solid rgba(239,68,68,0.4)` : `1px solid ${C.border}` }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '16px 18px' }}><div style={{ width: 52, height: 52, borderRadius: 16, fontSize: 26, background: canAfford ? C.greenLight : C.cardAlt, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{reward.emoji}</div><div style={{ flex: 1 }}><div style={{ fontWeight: 700, fontSize: 16, color: C.textPri }}>{reward.name}</div><div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 4 }}><span style={{ background: canAfford ? C.greenLight : C.cardAlt, color: canAfford ? C.green : C.textMut, borderRadius: 20, padding: '2px 10px', fontSize: 12, fontWeight: 700 }}>{reward.cost} pts</span>{!canAfford && <span style={{ fontSize: 11, color: C.textMut }}>{reward.cost - points} more needed</span>}</div></div><button onClick={() => claim(reward)} style={{ background: canAfford ? C.blue : C.cardAlt, color: canAfford ? '#fff' : C.textMut, border: 'none', borderRadius: 14, padding: '11px 20px', fontWeight: 700, fontSize: 14, cursor: 'pointer', flexShrink: 0 }}>Claim</button></div>
      {(isClaimed || isShort) && <div style={{ padding: '11px 18px', background: isClaimed ? C.greenLight : C.redLight, borderTop: `1px solid ${isClaimed ? 'rgba(34,197,94,0.2)' : 'rgba(239,68,68,0.2)'}`, display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ fontSize: 16 }}>{isClaimed ? '🎉' : '💪'}</span><span style={{ fontSize: 13, fontWeight: 600, color: isClaimed ? C.green : C.red }}>{isClaimed ? `Reward claimed! Enjoy your ${reward.name}.` : 'Earn one more tiny win first.'}</span></div>}
    </div>})}
  </div>
}
